      ...
      {slides.map((slide, index) => {
        ...
        const content = (
          <>
            ...
            <div className="relative z-20 px-4 sm:px-8 flex flex-col max-w-3xl mx-auto text-center items-center animate-fade-in pointer-events-none">
              <div className="bg-black/55 backdrop-blur-md px-6 py-6 sm:px-10 sm:py-8 rounded-2xl border border-[#d4af37]/40 shadow-2xl max-w-xl w-full flex flex-col items-center">
                <h2
                  className="text-xl sm:text-3xl md:text-4xl lg:text-5xl font-serif font-light tracking-[0.15em] uppercase text-shadow drop-shadow-md"
                  style={{ color: slide.title_color || "#d4af37" }}
                >
                  {slide.title}
                </h2>
                <p
                  className="text-xs sm:text-sm md:text-base font-serif italic tracking-wide mt-2 text-white/95"
                  style={{ color: slide.subtitle_color || "#ffffff" }}
                >
                  {slide.subtitle}
                </p>
                {slide.btn_text && (
                  <div className="flex justify-center items-center gap-4 pt-5 pointer-events-auto">
                    <span
                      className="text-[10px] sm:text-xs tracking-[0.2em] uppercase font-semibold px-8 py-3 border border-[#d4af37] bg-[#d4af37]/20 hover:bg-[#d4af37] hover:text-black transition-all duration-300 inline-block shadow-lg rounded-full"
                      style={{
                        borderColor: slide.btn_bg || "#d4af37",
